...
class HPEEvaluator:
    ...
    def __init__(self, name):
        ...
        self._anno_file = os.path.join(
            self._out_dir, "anno_hpe_{}.pkl".format(self._name)
        )
    ...
    def _generate_anno_file(self):
        ...
        for i in range(len(self._dataset)):
            if (i + 1) in np.floor(np.linspace(0, len(self._dataset), 11))[1:]:
                print(
                    "{:3.0f}%  {:6d}/{:6d}".format(
                        100 * i / len(self._dataset), i, len(self._dataset)
                    )
                )
    ...
    def evaluate(self, res_file, out_dir=None):
        ...
        log_file = os.path.join(
            out_dir, "hpe_eval_{}_{}.log".format(self._name, res_name)
        )
        ...
        hpe_curve_dir = os.path.join(
            out_dir, "hpe_curve_{}_{}".format(self._name, res_name)
        )
        ...
        results = {
            "absolute": {"mpjpe": mean_ab, "auc": auc_ab},
            "root-relative": {"mpjpe": mean_rr, "auc": auc_rr},
            "procrustes": {"mpjpe": mean_pa, "auc": auc_pa},
        }
